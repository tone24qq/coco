...
def _score_experiment(
    ...
    if name in {"ablation_no_retrieval", "ablation_no_logistic"}:
        return compose_final_score_from_components(
            table, _weights_for_experiment(name, base_weights), DynamicWeightConfig.from_mapping(None)
        )
    return compose_final_score_from_components(table, _weights_for_experiment(name, base_weights), dynamic_for_exp)
...
def main() -> None:
    ...
    log_progress(2, 7, "讀取 ranking dataset", f"input={args.input}")
    ...
    folds = run_cv(df, feature_cols, weights, dynamic_cfg=dynamic_cfg, n_splits=n_splits, min_train_issues=min_train_issues)
    log_progress(3, 7, "完成 walk-forward folds", f"folds={len(folds)}")
    ...
    for exp in experiments:
        ...
        for fold in folds:
            train_scored = _score_experiment(fold.train_scored, exp, weights, dynamic_cfg)
            val_scored = _score_experiment(fold.val_scored, exp, weights, dynamic_cfg)
    ...
    log_progress(4, 7, "完成 experiment runner", f"experiments={len(experiments)}")
    ...
    log_progress(5, 7, "輸出訓練/回測報表", "reports/")

    ranker, logistic = fit_models(df, feature_cols)
    scored_all = score_with_models(df, feature_cols, ranker, logistic, weights, dynamic_cfg=dynamic_cfg)
