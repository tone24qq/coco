...
from .manual_board_input import apply_overrides, load_manual_grid_with_states
...
def parse_manifest_entries(
    ...
    for entry in manifest:
        ...
        try:
            manual_grid, _ = load_manual_grid_with_states(entry.manual_grid)
            if manual_grid is not None:
                base_grid = manual_grid
                parse_diag = {"parse_policy": "manual_first_batch", "ocr_backend": "manual"}
                parse_conf = 1.0
            else:
                ...
                base_grid = [row[:] for row in result.grid]
                parse_diag = result.parse_diagnostics
                parse_conf = result.final_parse_confidence

            if entry.override:
                base_grid, _ = apply_overrides(base_grid, entry.override)

            if manual_grid is not None or entry.override:
                vals = [int(v) for row in base_grid for v in row if v is not None]
                result = BoardParseResult(
                    sample_id=entry.sample_id,
                    shape=f"{spec.expected_rows}x{spec.expected_cols}",
                    row_count=spec.expected_rows,
                    col_count=spec.expected_cols,
                    grid=base_grid,
                    cell_confidence=[
                        [1.0 for _ in range(spec.expected_cols)]
                        for _ in range(spec.expected_rows)
                    ],
                    low_confidence_cells=[],
                    parse_confidence=parse_conf,
                    image_path=entry.image_paths[0] if entry.image_paths else "",
                    ticket_type=entry.size_class,
                    ...
                    final_parse_confidence=parse_conf,
                    numbers_all=sorted(vals),
                    value_to_position={},
                    missing_values=sorted(list(spec.legal_values - set(vals))),
                    black_cells=[],
                    pending_cells=[],
            ...
            parse_conf = float(result.final_parse_confidence)
