...
def predict(payload: PredictPayload) -> dict:
    ...
    predict_compute_start = time.perf_counter()
    try:
        result = PREDICTOR.predict_from_draws(df, min_history=effective_min_history)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    predict_compute_elapsed_ms = int(
        (time.perf_counter() - predict_compute_start) * 1000
    )
    ...
    external_fetch_elapsed_ms = 0
    if payload.include_external_analysis:
        external_fetch_start = time.perf_counter()
        external = fetch_auzo_external_analysis(
            AuzoConfig(timeout_seconds=2.0, ttl_seconds=60)
        )
        external_fetch_elapsed_ms = int(
            (time.perf_counter() - external_fetch_start) * 1000
        )
    else:
        external = {
    ...
    result["latency_summary"] = {
        "predict_compute_elapsed_ms": predict_compute_elapsed_ms,
        "external_fetch_elapsed_ms": external_fetch_elapsed_ms,
        "total_api_elapsed_ms": int((time.perf_counter() - start) * 1000),
        "history_snapshot_load_elapsed_ms": int(
            result.get("history_prior_score_summary", {}).get(
                "snapshot_load_elapsed_ms", 0
            )
        ),
        "analysis_rerank_enabled": bool(
            result.get("analysis_rerank_summary", {}).get("enabled", False)
        ),
    }
