...
from src.retrieval import SimilarWindowRetriever, retrieval_features
...
def build_candidate_rows(
    ...
    retriever = SimilarWindowRetriever(window_size=retrieval_window, top_k=top_k)
    target_window = history[-retrieval_window:]
    lookup_history = history[:-1] if len(history) > retrieval_window + 1 else history
    matches = retriever.query(lookup_history, target_window, history[-1].day_issue_index)

    prof_10 = _history_profiles(history, 10)
    prof_20 = _history_profiles(history, 20)
    prof_n = _history_profiles(history, min(len(history), max(20, retrieval_window)))
    ...
    for cand in range(1, 81):
        ...
        row: dict[str, float | int | str] = {
            ...
            "normalized_day_progress": history[-1].day_issue_index / 200.0,
        ...
        row.update(prof_n)
        row.update(retrieval_features(matches, cand))
        rows.append(row)
    return rows


def build_feature_rows(records: list[DrawRecord], min_history: int, retrieval_window: int, top_k: int) -> list[dict[str, float | int | str]]:
    ...
    for idx in range(min_history, len(records)):
        ...
        rows.extend(
            build_candidate_rows(
                ...
                retrieval_window=min(retrieval_window, len(history)),
                top_k=top_k,
            )
        )
    return rows
...
def main() -> None:
    ...
    parser.add_argument("--retrieval-window", type=int, default=100)
    parser.add_argument("--top-k", type=int, default=20)
    args = parser.parse_args()

    records = read_processed(Path(args.input))
    rows = build_feature_rows(records, args.min_history, args.retrieval_window, args.top_k)
