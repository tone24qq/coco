...
def _score_experiment(
    table: pd.DataFrame, name: str, base_weights: RuntimeWeights, dynamic_cfg: DynamicWeightConfig
) -> pd.DataFrame:
    ...
    dynamic_for_exp = dynamic_cfg if has_dynamic_cols else DynamicWeightConfig.from_mapping(None)
    if name in {"ablation_no_retrieval", "ablation_no_logistic"}:
        return compose_final_score_from_components(table, weights, DynamicWeightConfig.from_mapping(None))
    return compose_final_score_from_components(table, weights, dynamic_for_exp)
...
def _alignment_audit(
    ...
    mainline_rescored_sample: pd.DataFrame | None,
    ...
    first = mainline_rescored_sample if mainline_rescored_sample is not None else pd.DataFrame()
    scoring_formula_match = False
    if not first.empty:
        expected = compose_final_score_from_components(first, weights, dynamic_cfg)["final_score"].round(12)
    ...
    return {
        ...
        "dynamic_weighting_enabled": dynamic_cfg.enabled,
        "scoring_backend": "shared_runtime_scoring",
...
def main() -> None:
    ...
    mainline_rescored_sample: pd.DataFrame | None = None
    for fold in folds:
        val_scored_by_exp: dict[str, pd.DataFrame] = {}
        for name in experiment_names:
            scored = _score_experiment(fold.val_scored, name, base_weights, dynamic_cfg)
        ...
        if main_exp_name in val_scored_by_exp:
            ...
            if mainline_rescored_sample is None:
                mainline_rescored_sample = val_scored_by_exp[main_exp_name]
            train_main = _score_experiment(fold.train_scored, main_exp_name, base_weights, dynamic_cfg)
    ...
    alignment = _alignment_audit(folds, feature_cols, df, base_weights, dynamic_cfg, mainline_rescored_sample)
