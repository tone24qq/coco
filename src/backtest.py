...
def main() -> None:
    ...
    regime_df.to_csv("reports/experiment_per_regime_metrics.csv", index=False)
    ablation_df.to_csv("reports/runtime_ablation_summary.csv", index=False)
    ...
    save_json(Path("reports/experiment_summary.json"), summary)
    save_json(Path("reports/alignment_audit.json"), {"time_series_split": True, "runtime_scoring_shared": True})
