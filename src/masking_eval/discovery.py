...
def run_module_discovery(
    ...
    for module in pool:
        ...
        keep = (delta_top1 > 0 or delta_top3 > 0) and delta_mrr > -0.005 and res["anti_leakage_checks"] == "passed"
    ...
    return {
        ...
        "leaderboard": sorted(leaderboard, key=lambda x: (x["top1"], x["top3"], x["mrr"]), reverse=True),
        "kept_modules": kept,
        "dropped_modules": [x["module"] for x in leaderboard if not x["keep"]],
        "champion": {
            "modules": champion_modules,
            "metrics": champion_result["full_model"],
            "best_weights": champion_result["best_weights"],
        },
    }


def write_discovery_outputs(result: Dict, leaderboard_path: Path, champion_path: Path, summary_path: Path) -> None:
    leaderboard_path.parent.mkdir(parents=True, exist_ok=True)
    leaderboard_path.write_text(json.dumps(result["leaderboard"], indent=2, ensure_ascii=False), encoding="utf-8")
    champion_path.write_text(json.dumps(result["champion"], indent=2, ensure_ascii=False), encoding="utf-8")
