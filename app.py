...
try:
    ...
    math_algo_kb = []
    logger.error(f"Failed to load knowledge base {kb_path}: {e}")
...
for hp in heatmap_paths:
    ...
    try:
        ...
        logger.info(f"Successfully loaded heatmap: {hp}")
    except (OSError, json.JSONDecodeError) as e:
        ...
        if file:
            ...
            try:
                ...
                if grid_array.ndim != 2:
                    logger.warning(f"JSON string contains non-2D grid, reshaping to 2D: {grid_array.shape}")
                    grid_array = grid_array.reshape(-1, 1) if grid_array.ndim == 1 else grid_array
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse JSON grid string: {e}")
