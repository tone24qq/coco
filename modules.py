...
class ScratchSolver:
    ...
    def idw_vectorized(self, grid):
        ...
        if self.tree is None or self.known_yx is None:
            return np.full(empty_yx.shape[0], 0.1)  # 最低信心分數
        ...
        est = np.where(est < 0.1, 0.1, est)  # 確保最低信心分數
        return est
    ...
    def compute_dynamic_hot_cold_advanced(self, grid, hot_q=0.9, cold_q=0.1, method='quantile'):
        known = grid[grid != -1]
        if known.size == 0:
            return np.full(np.count_nonzero(grid == -1), 0.1)
    ...
    def analyze_number_patterns(self, grid):
        ...
        def find_arithmetic(arr, min_len=3):
            if len(arr) < min_len:
                return None
            diffs = np.diff(arr)
    ...
    def fuse_scores_vectorized(self, mod_scores, board_type, default_weights):
        ...
        final = np.where(final < 0.1, 0.1, final)
        return final
