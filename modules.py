...
class AdaptiveWeights:
    ...
    def update(self, success_rate: float, module_scores: Dict[str, float]) -> None:
        ...
        for name in self.weights:
            ...
            self.weights[name] = max(0.1, min(0.9, self.history[name] * 1.05))  # Slight boost to adaptive weights
        total = sum(self.weights.values()) or 1e-10  # Prevent division by zero
        for name in self.weights:
