...
def simulate_with_formulas(grid_bytes: bytes, rows: int, cols: int, n_iter: int = 5_000_000, weights: Dict[str, float] = None) -> Dict[Tuple[int, int], Dict[int, float]]:
    ...
    batch_size = 10_000 if rows * cols < 50 else 5_000 if rows * cols < 200 else 1_000  # Optimized batch size for performance
    n_iter = int(os.getenv("ITER", 5_000_000)) if os.getenv("USE_FORMULA_ONLY") != "1" else 500_000  # Respect environment vars

    skip_scores = EXT_GM20_Skip_Pattern_Confidence_Vec(grid)
    grid_feats = compute_global_features(grid.astype(np.float32))
    mean_val, std_val = grid_feats[0], grid_feats[1] or 1.0  # Prevent division by zero
    ...
    for _ in range(n_iter // batch_size):
        ...
        valid &= np.array([np.corrcoef(skip_scores.ravel(), bs.ravel())[0, 1] > 0.85 for bs in board_scores])  # Increased threshold for better pattern matching
        ...
        for b_idx, board in enumerate(valid_boards):
            for r, c in blanks:
                ...
                resonance = 1 / (1 + abs(board[r, c] - known_vals.mean()) * 0.5) if len(known_vals) > 0 else 1.0  # Adjusted resonance factor
                global_weight = np.exp(-((board[r, c] - mean_val)**2) / (2 * (std_val**2 + 1e-6)))  # Added epsilon to avoid zero division
                hit_counter[(r, c)][board[r, c]] += board_scores[b_idx, r, c] * resonance * global_weight * 1.1  # Boost confidence weighting
            if all(max(cnt.values()) / sum(cnt.values()) > 0.95 for cnt in hit_counter.values()):
                break
    prob_map = {pos: {k: math_utils.normalize_value(v, min(cnt.values()) or 1e-10, max(cnt.values()) or 1e-10) for k, v in cnt.items()} for pos, cnt in hit_counter.items()}  # Prevent zero division
    return prob_map
    ...
    for r, c in blanks:
        ...
        if len(known_vals) > 0:
            for num, prob in prob_map[(r, c)].items():
                resonance = 1 / (1 + abs(num - known_vals.mean()) * 0.5)  # Adjusted resonance
                prob_map[(r, c)][num] *= resonance * 1.2  # Enhanced local weighting
            local_scores[r, c] = sum(prob_map[(r, c)].values())
    
    grid_feats = compute_global_features(grid.astype(np.float32))
    mean_val, std_val = grid_feats[0], grid_feats[1] or 1.0  # Default to 1.0 if std is zero
    for r, c in blanks:
        for num, prob in prob_map[(r, c)].items():
            global_weight = np.exp(-((num - mean_val)**2) / (2 * (std_val**2 + 1e-6)))  # Added epsilon
            prob_map[(r, c)][num] *= global_weight * 1.15  # Increased global influence

    skip_scores = EXT_GM20_Skip_Pattern_Confidence_Vec(grid)
    for r, c in blanks:
        for num, prob in prob_map[(r, c)].items():
            prob_map[(r, c)][num] *= skip_scores[r, c] * 1.1  # Boost pattern confidence
    ...
    for r, c in blanks:
        ...
        for num, prob in prob_map[(r, c)].items():
            if any(num in seq for seq in row_seqs + col_seqs):
                prob_map[(r, c)][num] *= 1.7  # Increased sequence boost for top-3 accuracy

    for pos in prob_map:
        total = sum(prob_map[pos].values()) or 1e-10  # Prevent division by zero
