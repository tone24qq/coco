...
def generate_full_boards(rows: int, cols: int, batch: int, rng: np.random.Generator, formulas: Tuple[str, ...]) -> np.ndarray:
    """Generate batch of complete boards using selected formulas."""
    n = rows * cols
    choices = rng.choice(formulas, size=batch, p=np.array([0.4, 0.3, 0.3]))
...
def weight_prob_by_modules(grid: np.ndarray,
    ...
    modules = [
        ...
        "EXT_GM20_Skip_Pattern_Confidence_Vec",
        "EXT_F10_Magnetic_Tail_Pattern_Vec"
    ]
    ...
    for (r, c), probs in result.items():
        ...
        neighbors = analyzer_utils.get_neighborhood_values(grid, r, c, radius=2, eight_connectivity=True)
        attraction_factor = 1.0
        if neighbors:
            mean_neighbor = np.mean(neighbors)
            attraction_factor = 1.0 + 0.2 * math_utils.sigmoid(abs(target_num - mean_neighbor) / 10.0) if target_num else 1.0

        if target_num is not None:
            base_prob = probs.get(target_num, 1e-10)
            adjusted_prob = base_prob * mean_score * attraction_factor
            total = adjusted_prob or 1e-10
            result[(r, c)] = {target_num: adjusted_prob / total}
        else:
            for val in probs:
                base_prob = probs[val]
                adjusted_prob = base_prob * mean_score * (1.0 + 0.1 * math_utils.sigmoid(abs(val - mean_neighbor) / 10.0) if neighbors else 1.0)
                probs[val] = max(adjusted_prob, 1e-10)
...
def mcts(grid: np.ndarray, iterations: int = 1000):
    ...
    def simulate(node):
        ...
        sim_result = simulate_full_board(current.grid, None, n_iter=500)
...
def predict_scratch_card(
    ...
    modules = [
        ...
        ("EXT_F10_Magnetic_Tail_Pattern_Vec", "Magnetic tail pattern")
    ]
    ...
    prob_map = simulate_full_board(grid_np, target_num, n_iter=total_iter)
