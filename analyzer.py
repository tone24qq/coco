    ...
    def check_pv(cls, pv, values):
        ...
        if grid:
            ...
            if pv.value < 1 or pv.value > maxv:
                raise ValueError(f"value 超出合法范围：1~{maxv}")
        return pv
...
def M10_edge(ctx):
    r,c = ctx.row, ctx.col
    if r in (0,ctx.rows-1) or c in (0,ctx.cols-1):
        return (1.0, "M10_edge")
    return (0.0, "")
...
def M14_mirror(ctx):
    ...
    twin = ctx.grid[ctx.row,mc]
    if twin != -1 and abs(ctx.val-twin)<=2:
        return ((2-abs(ctx.val-twin))/2, f"M14_mirror({twin})")
    return (0.0,"")
...
def M17_col_mir(ctx):
    mc = ctx.cols-1-ctx.col
    return (1.0, "M17_col_mir") if ctx.val == ctx.grid[ctx.row,mc] else (0.0,"")

def M18_hdelta(ctx):
    r,c=ctx.row,ctx.col
    left = ctx.grid[r,c-1] if c>0 else -1
    right= ctx.grid[r,c+1] if c<ctx.cols-1 else -1
    if left!=-1 and right!=-1 and abs(left-right)<=2:
        return ((2-abs(left-right))/2, f"M18_hdelta({left},{right})")
    return (0.0,"")
