...
def _as_model_input(flat: np.ndarray) -> np.ndarray:
    """Replace ``BLANK_VALUE`` with ``MASK_TOKEN_ID`` and ensure int64 array."""
    ...
    *,
    ...
    with jsonl_path.open("rb") as f:
        for idx, line in enumerate(f):
            ...
            mem_board = np.asarray(item["board"], dtype=int)
            mem_flat = mem_board.flatten()
            ...
            if len(heap) < k_neighbors:
                heapq.heappush(heap, (sim, idx, mem_board))
            elif sim > heap[0][0]:
                heapq.heapreplace(heap, (sim, idx, mem_board))

    if heap:
        mem_scores = np.zeros(num_cells, dtype=float)
        for _, _, mem_board in heap:
            mem_flat = mem_board.flatten()
            logits = model.forward(_as_model_input(mem_flat))
            if hasattr(logits, "detach"):
                arr = logits.detach().cpu().numpy()
            else:
                arr = np.asarray(logits)
