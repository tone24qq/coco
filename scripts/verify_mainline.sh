...
echo "[7/9] backtest mainline execution"
...
echo "[8/9] predict mainline execution"
python - <<'PY'
import json
from datetime import date, timedelta
from pathlib import Path
...
PY
python -m src.predict --config configs/predict.yaml --output reports/latest_prediction.json --recent-json /tmp/verify_recent.json

echo "[9/9] output assertions"
python - <<'PY'
import json
from pathlib import Path
import pandas as pd
