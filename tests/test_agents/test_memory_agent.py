...
class TorchModel(DummyModel):
    def forward(self, board_flat: np.ndarray):  # type: ignore[override]
        return torch.randn(self.num_cells, self.num_cells, requires_grad=True)
...
def test_memory_agent_handles_torch_tensor() -> None:
    rows, cols = 4, 5
    samples = load_samples_for_shape(rows, cols)
    model = TorchModel(num_cells=rows * cols, hidden_dim=8)
    memory_keys, memory_values = build_memory(samples, model)
    board, target = samples[0]
    res = predict(
        board.copy(),
