...
def test_auto_fetch_target_uses_latest_same_day_250(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("src.predict.probe_latest_same_day_issue", lambda timeout_s=10.0, sources=None: ("115016250", "winwin_dynamic"))
...
def test_auto_fetch_fail_fast_when_all_sources_partial_same_day(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("src.predict.run_source_consensus", fake_consensus)
    monkeypatch.setattr("src.predict.probe_latest_same_day_issue", lambda timeout_s=10.0, sources=None: ("115016240", "winwin_dynamic"))
    with pytest.raises(DataContractError, match="incomplete issue set"):
        _load_recent_draws(_cfg(tmp_path), None)


def test_auto_fetch_fail_fast_when_all_sources_stale_but_complete(monkeypatch, tmp_path) -> None:
    rows = _same_day_rows("115016", 240)
    ...
    with pytest.raises(DataContractError, match=r"freshness mismatch .*merged_same_day_issue_max=115016240.*authoritative_latest_issue=115016250"):
        _load_recent_draws(_cfg(tmp_path), None)
