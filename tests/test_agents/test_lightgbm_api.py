...
def test_predict_auto_fetch_when_recent_draws_missing(monkeypatch):
    ...
    assert body["records"]
    assert len(body["records"]) == body["recent_draws_count"]
    assert isinstance(body["records"][0]["numbers"], list)
    assert len(body["records"][0]["numbers"]) == 20
    assert "feature_mode" in body
    assert "degraded_features" in body
    assert "effective_windows" in body


def test_predict_auto_fetch_keeps_fallback_sources(monkeypatch):
    monkeypatch.setattr(api_module, "PREDICTOR", _StubPredictor())
    ...
    assert resp.status_code == 200
    assert captured["sources"][0] == "https://winwin.tw/Bingo"
    assert "https://www.pilio.idv.tw/bingo/list.asp" in captured["sources"]
...
def test_predict_success_contains_analysis_report(monkeypatch):
    ...
    assert "ranking_score_table" in body
