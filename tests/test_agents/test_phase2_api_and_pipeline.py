...
def test_health_and_predict_schema_with_mocked_runtime(monkeypatch, synthetic_records) -> None:
    ...
    monkeypatch.setattr(api_module, "get_runtime", lambda: (FakeArtifacts(), {"history": {"lookback_n": 100}, "auto_fetch": {"source": "x"}}, None))
    ...
    payload = {"recent_draws": [r.to_dict() for r in synthetic_records[-100:]]}
...
def test_health_degraded_when_artifacts_missing(monkeypatch):
    monkeypatch.setattr(api_module, "get_runtime", lambda: (None, {"history": {"lookback_n": 100}, "auto_fetch": {"sources": ["s"]}}, "missing"))
    client = TestClient(app)
