...
from src.grid_detector import GridDetectionError, detect_grid, infer_grid_shape
...
def test_infer_grid_shape_from_real_images() -> None:
    cases = [
        ("gogo/20/IS23120130.jpg", (5, 4)),
        ("gogo/80/NL230505019.jpg", (10, 8)),
    ]
    for path, expected in cases:
        gray = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        assert gray is not None
        shape, conf = infer_grid_shape(gray)
