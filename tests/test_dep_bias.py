...
from src.inference import api as inference_api
...
def test_predict_dep_bias_changes_probs(monkeypatch):
    class DummyModel(nn.Module):
        ...
        def forward(self, tokens, attn):  # type: ignore[override]
            B, L = tokens.shape
            return torch.zeros(B, L, 7)

    dummy = DummyModel()

    def fake_loader(rows, cols):
        return dummy

    monkeypatch.setattr(inference_api, "load_model_for_size", fake_loader)

    client = TestClient(inference_api.app)
    grid = [[1, -1, 2], [3, -1, 4]]
    resp = client.post("/predict", json={"grid": grid, "target": 2})
    assert resp.status_code == 200
    preds = {(p["row"], p["col"]): p["prob"] for p in resp.json()["predictions"]}
