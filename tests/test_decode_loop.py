...
from src.inference.decode import iterative_decode
...
class StubModel(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, attn_mask=None) -> torch.Tensor:  # type: ignore[override]
        B, M = tokens.shape
...
def test_iterative_decode_fills_grid() -> None:
    ...
    out = iterative_decode(model, tokens, attn, N=board.numel())
