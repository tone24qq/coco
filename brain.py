...
class BoardAnalyzerUtils:
    ...
    @staticmethod
    def get_arithmetic_or_geometric_sequences(line: np.ndarray, min_len: int = 3, allow_gaps: int = 1) -> List[List[int]]:
                ...
                if line[j] == -1:
                        ...
                        if line[k] == -1:
                            ...
                            if temp_gap <= allow_gaps:
                                ...
                                for l in range(k + 1, n):
                                    ...
                                    if MathUtils.isclose(line[l], expected, 1e-9):
                    ...
                    for k in range(j + 1, n):
                        if line[k] == -1:
                            ...
                            continue
                        expected = seq_vals[-1] + diff
                        if MathUtils.isclose(line[k], expected, 1e-9):
...
def EXT_M3_Local_Focus_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            std_val = np.std(neighbors, ddof=1) or 0.1
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            for offset in range(-min(sub_grid.shape[0], sub_grid.shape[1]), min(sub_grid.shape[0], sub_grid.shape[1])):
                ...
                diag_flipped = np.diagonal(np.flip(sub_grid, axis=1), offset)
                if len(diag_flipped) >= min(3, len(diag_flipped)):
                    diag_seqs.extend(utils.get_arithmetic_or_geometric_sequences(diag_flipped))
            legal_values = utils.get_legal_values_for_placement(grid)
            max_score = 0.0
            for val in legal_values:
                row_fit = any(val == v for seq in row_seqs for v in seq)
                col_fit = any(val == v for seq in col_seqs for v in seq)
                diag_fit = any(val == v for seq in diag_seqs for v in seq)
