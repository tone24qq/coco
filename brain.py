...
class MathUtils:
    ...
    def normalize_value(self, value: float, min_val: float, max_val: float, clamp: bool = True) -> float:
        ...
        強化: 處理 min_val 和 max_val 相等時，根據 value 與其的關係返回 0.0, 0.5, 或 1.0，更精確地處理邊界情況。
        """
...
class BoardAnalyzerUtils:
    ...
    def get_neighborhood_values(self, grid: np.ndarray, r: int, c: int, radius: int = 1,
                                ...
                                val_func: Callable[[int], Optional[float]] = lambda x_val: float(x_val) if x_val != -1 else None, [cite: 6]
    ...
    def get_value_gradient_at_cell(self, grid: np.ndarray, r: int, c: int,
                                   val_func: Callable[[int], float] = lambda x_val: float(x_val) if x_val != -1 else 0.0) -> Tuple[float, float]: [cite: 8]
        """
        ...
        gx = (safe_val(r - 1, c + 1) + 2 * safe_val(r, c + 1) + safe_val(r + 1, c + 1)) - \
             (safe_val(r - 1, c - 1) + 2 * safe_val(r, c - 1) + safe_val(r + 1, c - 1)) [cite: 8]
        gy = (safe_val(r + 1, c - 1) + 2 * safe_val(r + 1, c) + safe_val(r + 1, c + 1)) - \
             (safe_val(r - 1, c - 1) + 2 * safe_val(r - 1, c) + safe_val(r - 1, c + 1)) [cite: 8]
    ...
    def find_sequences_in_line(self, line: List[int], min_len: int = 3,
        ...
        強化: 提升算術序列檢測的彈性，能識別更多複雜的算術序列模式 (負公差，跨零點，常數序列的明確處理)。
        同時返回找到的序列、類型和公差/比率。
        """
        ...
        # Enhanced check for arithmetic sequences, considering constant non-zero as excluded by default for 'arithmetic'
        # but can be included if diff=0 is explicitly desired.
        for i in range(n):
            if line[i] == -1:
                continue
            ...
            if check_arithmetic:
                ...
                # Consider values up to min_len to establish a potential diff, then extend
                for j in range(i + 1, n):
                            ...
                            for k in range(j, n):
                                if line[k] == -1:
                                    ...
                                    if temp_gap_count <= allow_gaps:
                                        # Establish diff
                                        diff = line[k] - line[i]
                                        # If diff is 0 and non-zero number, consider it constant, not arithmetic for general purpose
                                        if diff == 0 and line[i] != 0: [cite: 10, 11]
                                            break # Not a strict arithmetic sequence
                                        ...
                                        for l in range(k + 1, n):
                                            ...
                                            elif line[l] != -1: [cite: 12] # Sequence broken by a different number
                                                break
                                        
                                        if len(current_seq_values) >= min_len:
                                            sequences.append(current_seq_values)
                                    break # Done trying to establish diff from j
                                else:
                                    break # Found non-gap number immediately after j, try to establish diff
                            break # Done with outer j loop for this i

                        continue # Move to next j
                    
                    diff = line[j] - line[i]
                    if diff == 0 and line[i] != 0: [cite: 10, 11] # Exclude constant non-zero sequences as arithmetic by default
                        continue
                    ...
                    for k in range(j + 1, n):
                        ...
                        elif line[k] != -1: [cite: 12] # Sequence broken by a different number
                            break
            ...
            if check_geometric and line[i] != 0:
                ...
                for j in range(i + 1, n):
                    ...
                    if line[j] == 0:
                        break  # Geometric sequences with zero are tricky, for simplicity, break

                    if ratio is None:
                        if line[i] == 0 and line[j] != 0: break  # 0, non-zero cannot be start of geom seq [cite: 13]
                        if line[i] != 0:
                            # If ratio isn't integer-like and not a trivial division [cite: 13]
                            if line[j] % line[i] != 0 and line[i] % line[j] != 0 and not math.isclose(line[j]/line[i], round(line[j]/line[i])): [cite: 13]
                                break
                            ratio = line[j] / line[i]
                        else:  # line[i] == 0 and line[j] == 0
                            ratio = 1.0  # Treat as constant 0s sequence for this part

                        if math.isclose(ratio, 1.0) and line[i] != line[j]: continue  # Avoid constant sequences if not identical [cite: 13]
...
def EXT_A2_Weighted_Proximity_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    radius = 2  # Consider a neighborhood radius [cite: 19]
    value_weight_factor = 0.15  # Weight factor for the value of neighboring numbers (slightly increased) [cite: 19]
    distance_decay_factor = 1.8  # Higher value means faster decay with distance [cite: 21]

    # --- Self-adaptive weights (Conceptual)
    # avg_grid_val = np.mean(grid[grid != -1]) if np.count_nonzero(grid != -1) > 0 else 0 [cite: 20]
    # if avg_grid_val > some_threshold: value_weight_factor *= 1.2 [cite: 20]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 20]
                continue
            ...
            for dr in range(-radius, radius + 1):
                for dc in range(-radius, radius + 1):
                    if dr == 0 and dc == 0: continue  # Skip center cell [cite: 21]

                    nr, nc = r_idx + dr, c_idx + dc

                    if 0 <= nr < rows and 0 <= nc < cols and grid[nr, nc] != -1:
                        dist = MathUtils().manhattan_distance((r_idx, c_idx), (nr, nc))
                        if dist == 0: dist = 1  # Safeguard for logic errors [cite: 22]
            ...
            # Improved heuristic for normalization: consider max value on board. [cite: 23]
            max_val_on_grid = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 23]
...
def EXT_M3_Local_Heterogeneity_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    all_possible_values_in_game = BoardAnalyzerUtils().get_all_possible_numbers_for_grid(grid.shape) [cite: 25]

    if not all_possible_values_in_game:
        return scores

    # Theoretical maximum entropy for normalization: log2 of the number of unique possible values. [cite: 26, 29]
    if len(all_possible_values_in_game) > 1:
        ...
        max_theoretical_entropy = math.log2(2)  # Avoid log2(1)=0, give some scale, or just 0 if N=1 means no diversity [cite: 28, 30, 31, 32]
    else:  # No possible values (empty set)
        max_theoretical_entropy = 1.0  # Fallback, though handled by early exit [cite: 29]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 29]
                continue
            ...
            # Normalizing by max_theoretical_entropy based on all game values gives a global perspective. [cite: 32, 36]
...
def EXT_D3_Potential_Field_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    decay_exponent = 1.5  # How quickly influence decays with distance (e.g., 1 for 1/r, 2 for 1/r^2) [cite: 34, 39]
    max_influence_radius = 3  # Consider numbers within this Manhattan distance [cite: 34, 39]

    # Max possible value on this specific grid for normalization scaling [cite: 34, 39]
    max_possible_val_on_grid = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 34]
    if max_possible_val_on_grid == 0: return scores  # Fallback

    # Heuristic maximum potential for normalization: [cite: 35, 40]
    # Sum of max_value / (min_dist^decay) for all cells in radius. [cite: 35, 40]
    num_cells_in_radius_approx = (2 * max_influence_radius + 1)**2 - 1
    heuristic_max_potential = num_cells_in_radius_approx * (max_possible_val_on_grid / (1**decay_exponent)) [cite: 35, 40]
    if heuristic_max_potential == 0: heuristic_max_potential = 1.0  # Avoid division by zero if somehow calculated as 0 [cite: 41]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 36]
                continue
            ...
            for nr in range(rows):
                for nc in range(cols):
                    if grid[nr, nc] != -1:  # If it's a filled cell (a "charge") [cite: 36]
                        num_val = grid[nr, nc]
                        if num_val <= 0: continue  # Consider only positive charges for attraction

                        dist = MathUtils().manhattan_distance((r_idx, c_idx), (nr, nc))
                        if dist == 0: continue  # Should not happen if only scoring empty cells [cite: 36]
                        if dist > max_influence_radius: continue  # Limit influence range

                        # Potential = charge_value / distance^decay_exponent [cite: 36, 42]
...
def EXT_F10_Discontinuity_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    legal_values_for_placement = BoardAnalyzerUtils().get_legal_values_for_placement(grid) [cite: 38]
    if not legal_values_for_placement:
        return scores

    min_sequence_len_to_score = 3  # Minimum length of a sequence to be considered significant [cite: 38, 43]
    # Heuristic max length for normalization (length of the longest possible line) [cite: 38, 43]
    heuristic_max_len = float(max(rows, cols))
    if heuristic_max_len < min_sequence_len_to_score:
        heuristic_max_len = float(min_sequence_len_to_score)  # Ensure max_len is at least min_len for normalization [cite: 38, 43]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 39]
                continue

            max_len_contribution_for_this_cell = 0.0  # Max sequence length this cell can contribute to [cite: 39, 44]
            ...
            for val_to_try in legal_values_for_placement:
                ...
                row_line = list(temp_grid[r_idx, :]) [cite: 40]
                sequences_in_row = BoardAnalyzerUtils().find_sequences_in_line(row_line,
                                                                               min_len=min_sequence_len_to_score,
                                                                               allow_gaps=1, check_arithmetic=True)  # Enhanced: allow 1 gap, only arithmetic for now [cite: 41]
                ...
                col_line = list(temp_grid[:, c_idx]) [cite: 42]
                sequences_in_col = BoardAnalyzerUtils().find_sequences_in_line(col_line,
                                                                               min_len=min_sequence_len_to_score,
                                                                               allow_gaps=1, check_arithmetic=True)  # Enhanced: allow 1 gap [cite: 42]
                ...
                sequences_in_diag1 = BoardAnalyzerUtils().find_sequences_in_line(diag1_line,
                                                                                 min_len=min_sequence_len_to_score,
                                                                                 allow_gaps=1, check_arithmetic=True)  # Enhanced: allow 1 gap [cite: 43]
...
def EXT_P7_Pathfinding_Value_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    legal_values_for_placement = BoardAnalyzerUtils().get_legal_values_for_placement(grid) [cite: 44]
    if not legal_values_for_placement:
        return scores

    max_path_search_depth = 4  # Max length of path to search [cite: 44, 51]
    path_value_decay_factor = 1.0  # Decay for path length, e.g., val / (len^decay) [cite: 44, 51]

    max_possible_val_on_grid = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 44]
    if max_possible_val_on_grid == 0: max_possible_val_on_grid = 1.0  # Avoid zero

    # Heuristic max score for normalization: [cite: 44, 51]
    # Assume connecting to max_val at dist 1 from multiple directions up to depth [cite: 45, 51]
    heuristic_max_path_score = ((2 * max_path_search_depth + 1)**2 * max_possible_val_on_grid / (1**path_value_decay_factor)) [cite: 45, 51]
    if heuristic_max_path_score == 0: heuristic_max_path_score = 1.0

    for r_start in range(rows):
        for c_start in range(cols):
            if grid[r_start, c_start] != -1:  # Only score empty cells (where we might place a number) [cite: 45, 52]
                continue

            max_score_for_this_cell = 0.0

            for val_to_try in legal_values_for_placement:
                # Simulate placing val_to_try. [cite: 46, 53] The original grid is used to find *existing* numbers. [cite: 47, 53]
                # The path itself can traverse other empty cells. [cite: 48, 54]
                current_placement_path_score = 0.0

                q = deque([((r_start, c_start), 0)])  # ((r, c), current_path_length_from_start) [cite: 48, 54]
                visited_for_bfs = set([(r_start, c_start)])  # Visited for this specific BFS starting at (r_start, c_start) [cite: 48, 54]

                # The BFS explores from the cell (r_start, c_start) *as if* val_to_try is placed there. [cite: 49, 55]
                # It seeks paths through *other currently empty cells* to reach *existing numbers on the original grid*. [cite: 50, 55]
                head_count = 0  # Safety break for BFS [cite: 50]
                ...
                while q and head_count < max_bfs_steps:
                    ...
                    for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:  # Corrected: (0,1) and (0,-1) [cite: 50]
                        ...
                        if 0 <= next_r < rows and 0 <= next_c < cols:
                            ...
                            if grid[next_r, next_c] != -1:
                                ...
                                # Do not add this to visited_for_bfs or queue, as it's a terminal number for this path segment. [cite: 51, 57]
                ...
                if current_placement_path_score > max_score_for_this_cell: [cite: 53]
...
def EXT_R5_Resource_Control_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    對高價值數字的獲取潜力等。[cite: 59]
    目的:偏好那些能夠鞏固盤面控制權,或獲取潛在高價值數字的空格。[cite: 59]
    啟發式類型:策略與控制 [cite: 59]
    輸出詮釋:分數越高表示該空格在填入數字後,對資源的控制(如行列完成度、高價值數字佔據)越強 [cite: 59]
    ...
    # If no numbers can be placed, this module might score based on structural control only. [cite: 54, 59]
    # However, value_capture_score would be low or zero. [cite: 54, 59]

    max_possible_val_on_grid = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 54]
    ...
    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 55]
                continue

            # 1. Row Completeness Score [cite: 56, 62]
            num_filled_in_row = np.count_nonzero(grid[r_idx, :] != -1)
            row_completeness_score = (num_filled_in_row + 1) / cols if cols > 0 else 0

            # 2. Column Completeness Score [cite: 57, 63]
            num_filled_in_col = np.count_nonzero(grid[:, c_idx] != -1)
            col_completeness_score = (num_filled_in_col + 1) / rows if rows > 0 else 0

            # 3. Value Capture Score (potential to place a high value) [cite: 58, 64]
            # This score reflects the value of placing the best possible *available* number here. [cite: 59]
            ...
            # Combine scores: Example weights, can be tuned [cite: 59, 65]
            # Weights for row control, column control, value capture [cite: 59, 65]
            w_row = 0.3
            w_col = 0.3
            w_val = 0.4
            ...
            combined_score = (w_row * row_completeness_score +
                              ...
                              w_val * value_capture_score) [cite: 59]
...
def EXT_GM1_Row_Control_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 61]
    # If no numbers can be placed, sum_score and seq_score contributions will be minimal or zero. [cite: 62, 67]

    avg_potential_num_to_place = 0.0
    if potential_numbers_to_place:
        avg_potential_num_to_place = np.mean(potential_numbers_to_place)

    max_val_board = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 62]
    ...
    for r_idx in range(rows):
        ...
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 63]
                continue

            # 1. Density Score: How full the row will be [cite: 63, 68]
            density_score = (num_filled_in_row + 1.0) / cols if cols > 0 else 0.0

            # 2. Value Contribution Score (Sum Score) [cite: 64, 69]
            potential_row_sum = sum_current_row_values + avg_potential_num_to_place
            # Max possible row sum: cols * max_val_board (simplistic heuristic) [cite: 64, 70]
            ...
            # 3. Sequence Completion Score (Enhanced: using BoardAnalyzerUtils.find_sequences_in_line) [cite: 65, 71]
            seq_score = 0.0
            temp_row_line = list(grid[r_idx, :])
            # Temporarily place avg_potential_num_to_place for evaluation
            temp_row_line[c_idx] = avg_potential_num_to_place

            sequences_found = BoardAnalyzerUtils().find_sequences_in_line(temp_row_line, min_len=3, allow_gaps=1, check_arithmetic=True) [cite: 66, 67]
            
            # Check if any sequence involves the placed 'avg_potential_num_to_place'
            max_seq_len_with_avg = 0
            for seq in sequences_found:
                if avg_potential_num_to_place in seq: # This might be inaccurate if avg_potential_num_to_place is not a real value
                    max_seq_len_with_avg = max(max_seq_len_with_avg, len(seq))

            if max_seq_len_with_avg >= 3:
                seq_score = MathUtils().normalize_value(float(max_seq_len_with_avg), 3, cols, clamp=True) # Normalize by row length
            elif max_seq_len_with_avg > 0: # If it forms a smaller sequence but still contributes
                 seq_score = 0.25 # Small bonus for contributing to any sequence pattern

            # Combine scores (weights can be tuned) [cite: 69, 72]
            w_density = 0.4
            w_sum = 0.3
            w_seq = 0.3
...
def EXT_GM2_Col_Flow_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    核心規則:評估在特定空格填入數字後,對該列的完成度、數值總和或序列形成的貢獻。 [cite: 71, 74]
    目的:偏好那些能增強單列控制力或形成有價值列模式的填補。[cite: 71, 74]
    啟發式類型:線性結構控制(列) [cite: 71, 74]
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 71]

    avg_potential_num_to_place = 0.0
    if potential_numbers_to_place:
        avg_potential_num_to_place = np.mean(potential_numbers_to_place)

    max_val_board = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 71]
    if max_val_board == 0: max_val_board = 1.0

    for c_idx in range(cols):
        current_col_values_list = [val for val in grid[:, c_idx] if val != -1] [cite: 71]
        num_filled_in_col = len(current_col_values_list)
        sum_current_col_values = sum(current_col_values_list)

        for r_idx in range(rows):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 72]
                continue

            # 1. Density Score: How full the column will be [cite: 72, 75]
            density_score = (num_filled_in_col + 1.0) / rows if rows > 0 else 0.0

            # 2. Value Contribution Score (Sum Score) [cite: 73, 76]
            ...
            if heuristic_max_col_sum > 0:
                sum_score = MathUtils().normalize_value(potential_col_sum, 0, heuristic_max_col_sum, clamp=True) [cite: 73, 77]

            # 3. Sequence Completion Score (Enhanced: using BoardAnalyzerUtils.find_sequences_in_line) [cite: 74]
            seq_score = 0.0
            temp_col_line = list(grid[:, c_idx])
            # Temporarily place avg_potential_num_to_place for evaluation
            temp_col_line[r_idx] = avg_potential_num_to_place

            sequences_found = BoardAnalyzerUtils().find_sequences_in_line(temp_col_line, min_len=3, allow_gaps=1, check_arithmetic=True) [cite: 75]
            ...
            if max_seq_len_with_avg >= 3:
                seq_score = MathUtils().normalize_value(float(max_seq_len_with_avg), 3, rows, clamp=True) # Normalize by col length
            elif max_seq_len_with_avg > 0: # If it forms a smaller sequence but still contributes
                 seq_score = 0.25 # Small bonus for contributing to any sequence pattern

            # Combine scores [cite: 76, 78]
            w_density = 0.4
            w_sum = 0.3
            w_seq = 0.3
...
def EXT_GM3_Adv_Connected_Comp_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    for r_start in range(rows):
        for c_start in range(cols):
            if visited_overall[r_start, c_start] or grid[r_start, c_start] != -1:  # Skip if already visited or not an empty cell [cite: 77]
                continue
            ...
            while q:
                ...
                # Explore 4-connectivity neighbors [cite: 78]
            ...
            # Assign this normalized area size score to all cells in the found component [cite: 79, 81]
...
def EXT_GM4_Spatial_Auto_Corr_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    potential_numbers = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 79]

    # Determine a hypothetical value to test placement with [cite: 79, 84]
    hypothetical_val_to_place: float
    if potential_numbers:
        hypothetical_val_to_place = float(np.median(potential_numbers))
    else:
        # If no numbers can be legally placed, use a generic mid-value for the board [cite: 80, 84]
        max_board_val = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 80]
        hypothetical_val_to_place = (1.0 + float(max_board_val)) / 2.0 if max_board_val > 0 else 0.5

    max_val_on_grid_for_norm = float(BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols))) [cite: 80]
    if max_val_on_grid_for_norm == 0: max_val_on_grid_for_norm = 1.0  # Avoid div by zero if grid is 0x0 or 1x0 etc. [cite: 80, 84]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 81]
                continue

            # Get actual numeric neighbors (non-1) [cite: 81, 84]
            neighbor_values = BoardAnalyzerUtils().get_neighborhood_values(
                grid, r_idx, c_idx, radius=1, eight_connectivity=True,
                val_func=lambda x: float(x) if x != -1 else None, [cite: 81]
                include_center=False
            )
            ...
            # Calculate the difference between the hypothetical placed value and the mean of its actual neighbors [cite: 81, 85]
            diff_hypothetical_to_mean_neighbors = abs(hypothetical_val_to_place - mean_neighbors)

            # Normalize this difference. [cite: 82, 85] Max possible difference is roughly max_val_on_grid. [cite: 82, 85]
            # Score for positive autocorrelation: 1.0 - normalized_difference [cite: 82, 85]
            # (smaller difference means more similar, thus higher positive autocorrelation score) [cite: 82, 85]
...
def EXT_GM5_Line_Completion_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    if rows == 0 or cols == 0 or min(rows, cols) < 1: return scores  # Need at least 1 cell. For lines of 3, need more. [cite: 83, 87]

    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 83]
    if not potential_numbers_to_place:
        return scores

    line_completion_score_map = {  # Define scores for different types of completions [cite: 83, 87]
        "identical_3": 0.6,  # Completing a line of 3 identical numbers [cite: 83, 87]
        "arithmetic_3_mend": 0.7,  # Mending an arithmetic sequence X, val, Y [cite: 83, 87]
    ...
    high_val_threshold = max_board_val * 0.7 if max_board_val > 0 else 10 # Define what counts as high value

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 84]
                continue
            ...
            for p_val in potential_numbers_to_place:
                ...
                # Check all 8 directions + center for line completion checks around (r_idx, c_idx) [cite: 84, 88]
                # Directions: Horizontal, Vertical, Diagonal (top-left to bottom-right), Anti-Diagonal (top-right to bottom-left) [cite: 84, 88]
                for dr, dc in [(0, 1), (1, 0), (1, 1), (1, -1)]:  # Horizontal, Vertical, Diag, Anti-Diag [cite: 84, 89]
                    if dr == 0 and dc == 0: continue  # Should not happen with this set of dr, dc [cite: 85]

                    # Case 1: Mending a line -> N1 - p_val - N2 [cite: 85, 89]
                    # N1 is at (r_idx - dr, c_idx - dc), N2 is at (r_idx + dr, c_idx + dc) [cite: 85, 89]
                    r_n1, c_n1 = r_idx - dr, c_idx - dc
                    r_n2, c_n2 = r_idx + dr, c_idx + dc

                    if 0 <= r_n1 < rows and 0 <= c_n1 < cols and \
                       0 <= r_n2 < rows and 0 <= c_n2 < cols: [cite: 86]

                        val_n1 = grid[r_n1, c_n1]
                        val_n2 = grid[r_n2, c_n2]

                        if val_n1 != -1 and val_n2 != -1:  # Both neighbors exist [cite: 86, 89]
                            # Check for 3 identical [cite: 86, 89]
                            ...
                            # Check for 3 arithmetic and not constant [cite: 87, 90]
                            if (val_n1 + val_n2) == 2 * p_val and abs(p_val - val_n1) > 0:
                                ...
                                if (val_n1 + p_val + val_n2) / 3 > high_val_threshold:  # Check if avg val is high
                    ...
                    # Case 2: Extending a line -> p_val - N1 - N2 [cite: 87, 90]
                    # N1 is at (r_idx + dr, c_idx + dc), N2 is at (r_idx + 2*dr, c_idx + 2*dc) [cite: 88, 90]
                    ...
                    if 0 <= r_n1_ext1 < rows and 0 <= c_n1_ext1 < cols and \
                       0 <= r_n2_ext1 < rows and 0 <= c_n2_ext1 < cols:
                        ...
                        if val_n1_ext1 != -1 and val_n2_ext1 != -1:
                            ...
                            if (p_val + val_n2_ext1) == 2 * val_n1_ext1 and abs(val_n1_ext1 - p_val) > 0: [cite: 88]
                                max_completion_score_for_cell = max(max_completion_score_for_cell, line_completion_score_map["arithmetic_3_extend"])

                    # Case 3: Extending a line (other end) -> N1 - N2 - p_val [cite: 89, 91]
                    # N1 is at (r_idx - 2*dr, c_idx - 2*dc), N2 is at (r_idx - dr, c_idx - dc) [cite: 89, 91]
                    ...
                    if 0 <= r_n1_ext2 < rows and 0 <= c_n1_ext2 < cols and \
                       0 <= r_n2_ext2 < rows and 0 <= c_n2_ext2 < cols: [cite: 90]
                        ...
                        if val_n1_ext2 != -1 and val_n2_ext2 != -1:
                            ...
                            if (val_n1_ext2 + p_val) == 2 * val_n2_ext2 and abs(val_n2_ext2 - val_n1_ext2) > 0: [cite: 90]
                                max_completion_score_for_cell = max(max_completion_score_for_cell, line_completion_score_map["arithmetic_3_extend"])

            scores[r_idx, c_idx] = MathUtils().normalize_value(max_completion_score_for_cell, 0, 1.0, clamp=True)  # Scores are already ~0-1 [cite: 92]
    return scores
...
def EXT_GM6_Symmetry_Potential_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    rows, cols = grid.shape
    scores = np.zeros((rows, cols), dtype=float) [cite: 91]

    if rows == 0 or cols == 0: return scores

    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 91]
    if not potential_numbers_to_place: [cite: 92]
        return scores
    ...
    symmetry_scores_map = {
        ...
        "main_diagonal": 0.6,  # (r,c) vs (c,r) - only if rows==cols for full meaning [cite: 92, 135]
        "anti_diagonal": 0.6,  # (r,c) vs (rows-1-c, cols-1-r) - only if rows==cols [cite: 92, 137]
    }
    ...
    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 93]
                continue
            ...
            for p_val in potential_numbers_to_place:
                ...
                # 1. Horizontal Symmetry: (r_idx, c_idx) vs (r_idx, cols - 1 - c_idx) [cite: 93, 131]
                sr_h, sc_h = r_idx, cols - 1 - c_idx
                if sc_h != c_idx and 0 <= sr_h < rows and 0 <= sc_h < cols and grid[sr_h, sc_h] == p_val: [cite: 93]
                    current_pval_max_sym = max(current_pval_max_sym, symmetry_scores_map["horizontal"])

                # 2. Vertical Symmetry: (r_idx, c_idx) vs (rows - 1 - r_idx, c_idx) [cite: 94, 132]
                sr_v, sc_v = rows - 1 - r_idx, c_idx
                if sr_v != r_idx and 0 <= sr_v < rows and 0 <= sc_v < cols and grid[sr_v, sc_v] == p_val: [cite: 94]
                    current_pval_max_sym = max(current_pval_max_sym, symmetry_scores_map["vertical"])

                # 3. Point (Center) Symmetry: (r_idx, c_idx) vs (rows-1-r_idx, cols-1-c_idx) [cite: 95, 133]
                sr_p, sc_p = rows - 1 - r_idx, cols - 1 - c_idx
                if (sr_p != r_idx or sc_p != c_idx) and \
                   0 <= sr_p < rows and 0 <= sc_p < cols and grid[sr_p, sc_p] == p_val: [cite: 95]
                    current_pval_max_sym = max(current_pval_max_sym, symmetry_scores_map["point_center"])

                # 4. Main Diagonal Symmetry (\): (r_idx, c_idx) vs (c_idx, r_idx) [cite: 96, 134]
                # Meaningful especially for square or near-square grids. [cite: 97, 135]
                if rows == cols:  # More strictly for square grids, can be relaxed [cite: 97, 135]
                    sr_d1, sc_d1 = c_idx, r_idx
                    if (sr_d1 != r_idx or sc_d1 != c_idx) and \
                       0 <= sr_d1 < rows and 0 <= sc_d1 < cols and grid[sr_d1, sc_d1] == p_val: [cite: 97]
                        current_pval_max_sym = max(current_pval_max_sym, symmetry_scores_map["main_diagonal"])

                # 5. Anti-Diagonal Symmetry (/): (r_idx, c_idx) vs (cols - 1 - c_idx, rows - 1 - r_idx) (for matrix indices) [cite: 98, 137]
                # This reflects across the anti-diagonal for square matrices. [cite: 99, 138]
                if rows == cols:  # More strictly for square grids [cite: 99, 138]
                    # For grid[r_idx, c_idx], symmetric position is ((rows-1)-c_idx, (cols-1)-r_idx) [cite: 101, 142]
                    sr_d2, sc_d2 = (rows - 1) - c_idx, (rows - 1) - r_idx # Corrected for NxN matrix where rows=cols
                    # Original logic was (cols - 1) - c_idx, (rows - 1) - r_idx, which is valid if (r,c) maps to (new_r, new_c) after flip
                    # Let's stick to (N-1)-c, (N-1)-r for N x N matrix
                    # sr_d2, sc_d2 = (cols - 1) - c_idx, (rows - 1) - r_idx [cite: 102, 143] # Assuming N=rows=cols
            ...
            scores[r_idx, c_idx] = MathUtils().normalize_value(max_symmetry_score_for_cell, 0, 1.0, clamp=True)  # Max of map is 0.8 [cite: 102]
    return scores
...
def EXT_GM7_Numeric_Gaps_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    差為1的序列)。[cite: 144]
    強化: 增加對任意公差算術序列間隙填充的識別，並根據序列特徵給予不同分數。
    """
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 103]
    if not potential_numbers_to_place:
        return scores

    gap_fill_scores_map = {
        "arithmetic_1_gap_fill": 0.9,  # Fills X, p_val, X+2 (i.e. p_val = X+1) [cite: 103, 145]
        "arithmetic_generic_mend": 0.7,  # Fills X, p_val, Y where X, p_val, Y is arithmetic [cite: 103, 145]
        "arithmetic_generic_extend": 0.5,  # p_val, X, Y or X, Y, p_val is arithmetic [cite: 103, 145]
        # Added: scoring for quality (conceptual)
    ...
    max_board_val = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols))
    high_val_threshold = max_board_val * 0.7 if max_board_val > 0 else 10

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 104]
                continue
            ...
            for p_val in potential_numbers_to_place:
                ...
                # Iterate over 4 directions (Horizontal, Vertical, Main Diagonal, Anti-Diagonal) [cite: 104, 146]
                for dr, dc in [(0, 1), (1, 0), (1, 1), (1, -1)]:

                    # Case 1: p_val mends a gap: N1 - p_val - N2 [cite: 104, 146]
                    r_n1, c_n1 = r_idx - dr, c_idx - dc
                    r_n2, c_n2 = r_idx + dr, c_idx + dc
                    ...
                    if 0 <= r_n1 < rows and 0 <= c_n1 < cols and \
                       0 <= r_n2 < rows and 0 <= c_n2 < cols:
                        ...
                        if val_n1 != -1 and val_n2 != -1: [cite: 105]
                            # Specific check for arithmetic sequence with common difference 1 [cite: 105, 146]
                            if val_n1 == p_val - 1 and val_n2 == p_val + 1:
                                score = gap_fill_scores_map["arithmetic_1_gap_fill"]
                                # --- Quality Enhancement (Conceptual) ---
                                # If values are high, add bonus
                                if (val_n1 + p_val + val_n2) / 3 > high_val_threshold:
                            ...
                            # Generic arithmetic sequence check (d!=0) [cite: 105, 147]
                    ...
                    # Case 2: p_val extends a sequence: p_val - N1 - N2 [cite: 105, 147]
                    r_n1_ext1, c_n1_ext1 = r_idx + dr, c_idx + dc
                    r_n2_ext1, c_n2_ext1 = r_idx + 2 * dr, c_idx + 2 * dc [cite: 106]
                    ...
                    if 0 <= r_n1_ext1 < rows and 0 <= c_n1_ext1 < cols and \
                       0 <= r_n2_ext1 < rows and 0 <= c_n2_ext1 < cols:
                        ...
                        if val_n1_ext1 != -1 and val_n2_ext1 != -1: [cite: 107]
                            # Check for N1=p_val+d, N2=p_val+2d -> val_n1_ext1 - p_val == val_n2_ext1 - val_n1_ext1 (d) [cite: 107, 148]
                            # Check for: p_val, val_n1_ext1, val_n2_ext1 is arithmetic
                    ...
                    # Case 3: p_val extends a sequence: N1 - N2 - p_val [cite: 107, 148]
                    r_n1_ext2, c_n1_ext2 = r_idx - 2 * dr, c_idx - 2 * dc
                    r_n2_ext2, c_n2_ext2 = r_idx - dr, c_idx - dc
                    ...
                    if 0 <= r_n1_ext2 < rows and 0 <= c_n1_ext2 < cols and \
                       0 <= r_n2_ext2 < rows and 0 <= c_n2_ext2 < cols:
                        ...
                        if val_n1_ext2 != -1 and val_n2_ext2 != -1: [cite: 108]
                            # Check for N2=N1+d, p_val=N2+d -> val_n2_ext2 - val_n1_ext2 == p_val - val_n2_ext2 (d) [cite: 108, 149]
                            # Check for: val_n1_ext2, val_n2_ext2, p_val is arithmetic
                            if (val_n2_ext2 - val_n1_ext2) == (p_val - val_n2_ext2) and (val_n2_ext2 - val_n1_ext2) != 0: [cite: 109, 149]
                                max_cell_gap_score = max(max_cell_gap_score, gap_fill_scores_map["arithmetic_generic_extend"])

                if current_pval_score > max_cell_gap_score:
                    max_cell_gap_score = current_pval_score

            scores[r_idx, c_idx] = MathUtils().normalize_value(max_cell_gap_score, 0, 1.0, clamp=True)  # Scores are already ~0-1 [cite: 110, 150]
    return scores
...
def EXT_GM8_Edge_Affinity_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    輸出詮釋:分數高低取決於設定(偏好/避開邊緣)。預設偏好邊緣,越靠近邊緣/角落分數越高。[cite: 151]
    ...
    corner_bonus_prefer = 0.2  # Bonus if preferring edges and it's a corner [cite: 111, 151]
    corner_penalty_avoid = 0.2  # Penalty if avoiding edges and it's a corner [cite: 111, 151]

    # Max possible minimum distance to an edge (for normalization) [cite: 111, 152]
    # This would be for a cell at the center of the board. [cite: 112, 152]
    max_min_dist_to_edge_row = (rows - 1) // 2 if rows > 0 else 0 [cite: 113, 153]
    max_min_dist_to_edge_col = (cols - 1) // 2 if cols > 0 else 0 [cite: 113, 153]

    # The actual maximum of minimum distances to any edge. [cite: 114, 154]
    # e.g. 5x5 grid, center (2,2), min_dist = 2. max_min_dist = 2. [cite: 114, 154]
    # e.g. 5x3 grid, center-ish (2,1), min_dist_r = 2, min_dist_c = 1. [cite: 115] min_dist=1. [cite: 115, 155]
    overall_max_of_min_distances = float(min(max_min_dist_to_edge_row, max_min_dist_to_edge_col)) [cite: 117]

    if overall_max_of_min_distances == 0 and (rows > 1 or cols > 1):  # e.g. a 1xN or Nx1 line, max_min_dist is 0. [cite: 118, 157]
        overall_max_of_min_distances = 0.5  # Avoid div by zero for normalization if all cells are on edge [cite: 118, 157]
    elif rows <= 1 and cols <= 1: # For 1x1 grid, center is also edge, min_dist=0
        overall_max_of_min_distances = 1.0 # Ensure normalization divisor is not zero for 1x1 grid

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 118, 157]
                continue
            ...
            if overall_max_of_min_distances > 0:
                ...
                normalized_dist = min(1.0, normalized_dist)  # Clamp if min_dist somehow exceeds expected max [cite: 119, 158]
            elif min_dist == 0:  # all cells are on an edge, min_dist is 0 [cite: 119, 158]
                normalized_dist = 0.0
            else:  # Should not happen if overall_max_of_min_distances is handled [cite: 119, 158]
                normalized_dist = 1.0

            if affinity_mode == "prefer_edge":
                current_score = 1.0 - normalized_dist  # Closer to edge (smaller dist) -> higher score [cite: 119, 159]
                if is_corner and min_dist == 0:
                    current_score += corner_bonus_prefer
            elif affinity_mode == "avoid_edge":
                current_score = normalized_dist  # Further from edge (larger dist) -> higher score [cite: 119, 159]
                if is_corner and min_dist == 0:
                    current_score -= corner_penalty_avoid

            scores[r_idx, c_idx] = MathUtils().normalize_value(current_score,
                                                               -corner_penalty_avoid, 1.0 + corner_bonus_prefer, clamp=True) [cite: 120]
    return scores
...
def EXT_GM9_Center_Control_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    # Max possible distance from any cell to the center is the distance from a corner to the center. [cite: 121, 162]
    # Using (0,0) as the reference corner. [cite: 121, 162]
    max_dist_to_center = MathUtils().euclidean_distance((0.0, 0.0), (center_r, center_c)) [cite: 121]

    if max_dist_to_center == 0 and (rows > 1 or cols > 1):  # e.g. a 2x2 grid, center is (0.5,0.5), max_dist > 0 [cite: 122, 163]
        # This case is for 1x1 grid where center is (0,0) and max_dist is 0. [cite: 123, 164]
        # Or for any grid, if calculation is 0, set a small positive to avoid div by zero for normalization. [cite: 123, 164]
        max_dist_to_center = 1.0  # Avoid division by zero in normalization if grid is 1x1 [cite: 123, 164]
    elif max_dist_to_center == 0 and rows <= 1 and cols <= 1:  # Truly a 1x1 or 0x0 grid [cite: 124, 165]
        pass  # max_dist_to_center remains 0, normalization will handle.

    for r_idx in range(rows): [cite: 124]
        for c_idx in range(cols): [cite: 125]
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 125, 165]
                continue
            ...
            elif current_dist_to_center == 0:  # For 1x1 grid, dist is 0, max_dist is 0. Should be 0.5 normalized. [cite: 126, 166]
                normalized_dist = 0.0  # MathUtils.normalize_value will return 0.5 if val=min=max. [cite: 127, 166]
                # But if we want 0 dist to mean "perfectly at center", then 0 is fine here. [cite: 128, 166]
                # The score logic below will invert this. [cite: 128, 166]

            current_score = 0.0
            if affinity_mode == "prefer_center":
                current_score = 1.0 - normalized_dist  # Closer to center (smaller dist) -> higher score [cite: 128, 167]
            elif affinity_mode == "avoid_center":
                current_score = normalized_dist  # Further from center (larger dist) -> higher score [cite: 128, 167]
...
def EXT_GM10_Blocking_Value_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    目的:偏好那些不會導致形成不良結構的填補,或者理想情況下能主動阻止潛在不良結構形成的填補。[cite: 168]
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 129]
    if not potential_numbers_to_place:
        return scores

    # Define some simple undesirable sequences (length 3 for this example) [cite: 129, 169]
    # In a real system, these could be learned or more complex. [cite: 130, 170]
    UNDESIRABLE_SEQUENCES = [
        [1, 1, 1],  # Three 1s in a row [cite: 130, 170]
        [2, 2, 2],  # Three 2s in a row [cite: 130, 170]
        # [1, 2, 3], # Example: if a short ascending sequence is bad in some contexts [cite: 130, 170]
        # [5, 5, -1] # A partial bad pattern that could be completed (more complex to check "blocking") [cite: 130, 170]
    ]

    # For simplicity, this implementation checks if placing a number *completes* an undesirable sequence. [cite: 131, 171]
    # A high score means the placement *avoids* completing such sequences. [cite: 132, 171]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 133, 172]
                continue

            # Score for this cell will be the max score achievable by placing any potential number [cite: 133, 173]
            # where "score" means "does not complete an undesirable pattern". [cite: 133, 173]
            max_safety_score_for_cell = 0.0  # Default to low score if all placements are bad [cite: 133, 173]

            if not potential_numbers_to_place:  # If somehow list is empty now [cite: 133, 173]
            ...
            for p_val in potential_numbers_to_place:
                ...
                # Directions: Horizontal, Vertical, Main Diagonal, Anti-Diagonal [cite: 133, 173]
                for dr_line, dc_line in [(0, 1), (1, 0), (1, 1), (1, -1)]:
                    # Check 3 positions for each line: p_val in pos 0, 1, or 2 [cite: 134, 173]
                    for offset in range(-2, 1):  # Start of a 3-cell window [cite: 134, 173]
                        current_line_values = []
                        valid_line = True
                        line_coords = [] # Track coords for validation

                        for i in range(3):
                            check_r, check_c = r_idx + (offset + i) * dr_line, c_idx + (offset + i) * dc_line

                            if 0 <= check_r < rows and 0 <= check_c < cols: [cite: 134]
                        ...
                        if valid_line and len(current_line_values) == 3: [cite: 135]
                            # Ensure the currently placed p_val at (r_idx, c_idx) is part of this line [cite: 137, 179]
                ...
                if current_score_for_pval > max_safety_score_for_cell: [cite: 138]
                    max_safety_score_for_cell = current_score_for_pval

            scores[r_idx, c_idx] = max_safety_score_for_cell [cite: 138]
    return scores
...
def EXT_GM11_Pair_Correlation_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 139]
    if not potential_numbers_to_place:
        return scores

    # Define some favorable pairs and their scores. [cite: 140, 182]
    # (val1, val2) means val1 placed next to existing val2. [cite: 141, 183]
    # These could be learned or be part of a more complex configuration. [cite: 142, 184]
    FAVORABLE_PAIRS_SCORES = {
        (3, 7): 0.8, (7, 3): 0.8,  # 3 and 7 like to be together [cite: 142, 184]
        (1, 2): 0.6, (2, 1): 0.6,  # Sequential small numbers [cite: 142, 184]
        (10, 20): 0.7, (20, 10): 0.7,  # Example of decade pairing [cite: 142, 184]
        (5, 10): 0.5, (10, 5): 0.5,
        # Max value in game for mid-range consecutive
        (max(1, BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) // 2),
         max(1, BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) // 2) + 1): 0.4, [cite: 142, 184]
        (max(1, BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) // 2) + 1,
         max(1, BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) // 2)): 0.4,
    }
    ...
    # Heuristic max possible score: if all 8 neighbors form max-scoring pairs [cite: 143, 185]
    heuristic_max_total_pair_score = 8.0 * max_single_pair_score if max_single_pair_score > 0 else 1.0

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 143]
                continue
            ...
            for p_val in potential_numbers_to_place:
                ...
                # Check 8 neighbors [cite: 143, 185]
                for dr in [-1, 0, 1]:
                    for dc in [-1, 0, 1]:
                        if dr == 0 and dc == 0:
                            continue
                        ...
                        if 0 <= nr < rows and 0 <= nc < cols:
                            ...
                            if neighbor_val != -1:  # If neighbor is an existing number [cite: 144, 186]
                                # Check if (p_val, neighbor_val) is a favorable pair [cite: 144, 186]
...
def EXT_GM12_Island_Analysis_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    max_val_on_board = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 146]
    if max_val_on_board == 0: max_val_on_board = 1.0  # Avoid div by zero [cite: 146, 188]

    # Weights for combining island characteristics [cite: 146, 188]
    w_size = 0.4
    w_compactness = 0.3
    w_avg_value = 0.3
    ...
    for r_start in range(rows):
        for c_start in range(cols):
            if grid[r_start, c_start] != -1 and not visited_island_search[r_start, c_start]:  # Found an unvisited number [cite: 146, 188]
                ...
                while q:
                    ...
                    min_r_bbox = min(min_r_bbox, r_curr) [cite: 147]
                    max_r_bbox = max(max_r_bbox, r_curr)
                    min_c_bbox = min(min_c_bbox, c_curr)
                    max_c_bbox = max(max_c_bbox, c_curr)

                    for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:  # 4-connectivity for islands [cite: 147]
                        nr, nc = r_curr + dr, c_curr + dc

                        if 0 <= nr < rows and 0 <= nc < cols and \
                           grid[nr, nc] != -1 and not visited_island_search[nr, nc]: [cite: 148, 190]
                            visited_island_search[nr, nc] = True
                            q.append((nr, nc))

                # Calculate island characteristics [cite: 148, 190]
                ...
                compactness = 0.0
                if bbox_area > 0:
                    compactness = island_size / bbox_area  # (Ratio of actual cells to bounding box area) [cite: 148, 190]

                # Normalize characteristics [cite: 148, 190]
                ...
                # Combine into a single island score [cite: 149, 191]
                island_score = (w_size * norm_size +
                                w_compactness * norm_compactness +
                                w_avg_value * norm_avg_value) [cite: 149]

                final_island_score = MathUtils().normalize_value(island_score, 0, 1.0, clamp=True)

                # Assign this score to all cells in the current island [cite: 149, 191]
                for r_cell, c_cell in current_island_cells:
                    scores[r_cell, c_cell] = final_island_score

            elif grid[r_start, c_start] == -1:  # Empty cells get 0 score from this module [cite: 149, 191]
...
def EXT_GM13_Sequence_Diversity_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    """
    (GM13-序列多樣性) [cite: 192]
    核心規則:評估填補位置是否有助於形成多樣化的短序列(例如,不同方向、不同類型),而非僅專注於單一長序列。[cite: 192]
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 150]
    if not potential_numbers_to_place:
        return scores

    short_sequence_len = 3  # Define length of "short sequences" [cite: 150, 193]
    heuristic_max_distinct_sequences = 8.0  # Max distinct short sequences a single cell might participate in (heuristic for normalization) [cite: 150, 193]
    # For 4 directions, each can have arithmetic/identical from 3 positions (mend/extend). So 4*2*3 = 24 theoretical max types, but many overlap.
    # 8.0 is a reasonable heuristic.

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 151]
                continue
            ...
            for p_val in potential_numbers_to_place:
                ...
                found_sequence_signatures = set()  # Store signatures like ("arithmetic", (dr,dc), diff) or ("identical", (dr,dc), val) [cite: 151, 193]

                # Check in 4 directions (H, V, D1, D2) [cite: 151, 194]
                # Each direction vector also defines the line orientation [cite: 151, 194]
                for dr_dir, dc_dir in [(0, 1), (1, 0), (1, 1), (1, -1)]:

                    # For each direction, check short_sequence_len possible alignments of a length-short_sequence_len sequence where p_val is involved [cite: 151, 194]
                    for i_offset in range(short_sequence_len):  # p_val is at index i_offset in the 3-number window [cite: 151, 194]
                        current_sequence_values = []
                        valid_segment = True

                        for k_seq in range(short_sequence_len):
                            # Position of k_seq-th element in the window [cite: 152, 195]
                            # Relative to (r_idx, c_idx), this element is at (k_seq - i_offset) * (dr_dir, dc_dir) [cite: 152, 195]
                            check_r = r_idx + (k_seq - i_offset) * dr_dir [cite: 152]
                        ...
                        if valid_segment and len(current_sequence_values) == short_sequence_len:
                            ...
                            # 1. Arithmetic sequence (non-constant) [cite: 153]
                            if all(val != -1 for val in s):  # Ensure all are numbers [cite: 153]
                                diff1 = s[1] - s[0]
                                diff2 = s[2] - s[1]

                                if diff1 == diff2 and diff1 != 0: [cite: 153]
                                    found_sequence_signatures.add(("arithmetic", (dr_dir, dc_dir), diff1))

                                # 2. Identical sequence [cite: 154]
...
def EXT_GM14_Risk_Assessment_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    """
    (GM14 - 風險評估) [cite: 197]
    核心規則:評估某個填補動作的潛在「風險」,例如是否會導致後續選擇過少(降低盤面靈活性)。[cite: 197]
    ...
    initial_potential_numbers = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 155]
    if not initial_potential_numbers: [cite: 198]
    ...
    # max_possible_options = (rows * cols) * (rows * cols) # This is too large if used directly for normalization

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 156, 198]
                continue

            max_flexibility_score_for_cell = 0.0

            for p_val in initial_potential_numbers:  # Only try values that are currently legal for the original grid [cite: 157, 200]
                temp_grid = grid.copy()
                temp_grid[r_idx, c_idx] = p_val

                # Calculate flexibility after this placement [cite: 158, 202]
                # remaining_empty_cells = np.count_nonzero(temp_grid == -1)
                subsequent_legal_moves = len(BoardAnalyzerUtils().get_legal_values_for_placement(temp_grid)) [cite: 158]

                # Flexibility metric: just the number of subsequent legal moves [cite: 159, 202]
            ...
            # Normalize: Max possible subsequent_legal_moves is roughly rows*cols (if all cells are empty) [cite: 159, 203]
            current_max_heuristic_flex = float(rows * cols - 1)  # Max legal values after 1 placement [cite: 160, 203]
...
def EXT_GM15_Information_Gain_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 161]
    if not potential_numbers_to_place:
        return scores

    # Calculate entropy of the initial grid (all cells, -1 is a symbol) [cite: 161]
    initial_grid_values = [int(val) for val in grid.flatten()] [cite: 162]
    entropy_before = MathUtils().get_entropy(initial_grid_values)

    # Max possible entropy for normalization (log2 of number of symbols: 1 to R*C plus -1) [cite: 162]
    num_symbols = rows * cols + 1 # Include -1 as a symbol
    max_possible_entropy_change = math.log2(num_symbols) if num_symbols > 1 else 1.0 [cite: 162]
    # Max possible entropy itself [cite: 162]
    if max_possible_entropy_change == 0: max_possible_entropy_change = 1.0

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 162]
                continue

            # We want to maximize reduction, so initialize with a very low value
            ...
            for p_val in potential_numbers_to_place:
                ...
                entropy_reduction = entropy_before - entropy_after  # Higher reduction is better [cite: 163]
            ...
            # Normalize the reduction. [cite: 164] Min reduction can be negative (entropy increases). Max can be entropy_before. [cite: 164]
            # Score will be higher for positive reductions. [cite: 165]
            # Range of reduction is roughly [-max_possible_entropy_change, max_possible_entropy_change] [cite: 165]
            # For normalization, we want to map [min_possible_reduction, max_possible_reduction] to [0,1].
            # A good heuristic is to normalize positive reductions from 0 to max_possible_entropy_change.
            # And clamp negative reductions to 0 or a very small value.
            scores[r_idx, c_idx] = MathUtils().normalize_value(max_entropy_reduction_for_cell, 0, max_possible_entropy_change, clamp=True)
            # Clamping at 0 if it increases entropy (reduces score) [cite: 166]
    return scores
...
def EXT_GM16_Harmonic_Centrality_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    if rows == 0 or cols == 0 or (rows * cols) <= 1: return scores  # Needs more than 1 cell [cite: 167]

    # Max possible harmonic centrality (heuristic): if a cell is at distance 1 from all N-1 other cells. [cite: 167]
    max_hc_heuristic = float(rows * cols - 1) [cite: 167]
    if max_hc_heuristic == 0: max_hc_heuristic = 1.0

    for r_eval in range(rows):
        for c_eval in range(cols):
            if grid[r_eval, c_eval] != -1:  # Only score empty cells [cite: 167]
                continue

            current_harmonic_centrality = 0.0
            num_other_nodes = 0

            for r_other in range(rows): [cite: 168]
                for c_other in range(cols):
                    if r_eval == r_other and c_eval == c_other:
                        continue  # Skip self

                    # Using Manhattan distance as grid distance [cite: 168]
                    dist = MathUtils().manhattan_distance((r_eval, c_eval), (r_other, c_other)) [cite: 168]

                    if dist > 0:
                        current_harmonic_centrality += 1.0 / dist
                        num_other_nodes += 1

            if num_other_nodes == 0:  # Only one cell in grid, should have been caught earlier [cite: 168]
                scores[r_eval, c_eval] = 0.0
            else:
                # Normalization can be tricky. [cite: 169] The sum of reciprocals can vary. [cite: 169]
                # Using the heuristic max. [cite: 170]
...
def EXT_GM17_Entropy_Minimization_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 170]
    if not potential_numbers_to_place:
        return scores

    radius = 1  # Define local neighborhood radius [cite: 170]
    num_cells_in_neighborhood = (2 * radius + 1)**2  # Including center
    max_local_entropy_change = math.log2(num_cells_in_neighborhood) if num_cells_in_neighborhood > 1 else 1.0 [cite: 170]
    if max_local_entropy_change == 0: max_local_entropy_change = 1.0

    # Helper for entropy calculation that includes -1 as a symbol
    def val_func_for_entropy(x_val: int) -> int: return int(x_val)

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 171]
                continue

            # Calculate entropy of local neighborhood *including* the current cell (r_idx, c_idx)
            # with (r_idx, c_idx) treated as an empty cell (-1) for "before" calculation. [cite: 179]
            values_before_placement_local = BoardAnalyzerUtils().get_neighborhood_values(
                grid, r_idx, c_idx, radius=radius, eight_connectivity=True,
                val_func=val_func_for_entropy, include_center=True [cite: 179]
            )
            ...
            for p_val in potential_numbers_to_place:
                ...
                temp_grid_local_place[r_idx, c_idx] = p_val [cite: 180]

                # Calculate entropy of local neighborhood *including* p_val [cite: 173]
                values_after_placement_local = BoardAnalyzerUtils().get_neighborhood_values(
                    ...
                    val_func=val_func_for_entropy, include_center=True [cite: 180]
                )
            ...
            if max_entropy_reduction_for_cell == -float('inf'):
                max_entropy_reduction_for_cell = 0.0 [cite: 180]
...
def EXT_GM18_RL_Value_Est_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    此為簡化版,模擬從歷史數據學習到的偏好。
    目的:偏好那些能夠使盤面展現更多理想特徵(如形成特定序列、達到特定盤面密度等)的填
    補。
    ...
    potential_numbers_to_place = list(BoardAnalyzerUtils().get_legal_values_for_placement(grid)) [cite: 180]
    if not potential_numbers_to_place: [cite: 181]
        return scores

    # Define desirable features and their heuristic scores/weights [cite: 181]
    # These simulate a learned value function. [cite: 182]
    FEATURE_WEIGHTS = {
        "identical_3": 1.0,  # Completing a line of 3 identical numbers [cite: 181]
        "arithmetic_3": 0.7,  # Completing an arithmetic sequence of 3 [cite: 181]
        "board_density_factor": 0.2,  # General preference for denser boards (more numbers) [cite: 181]
        # Added conceptual features
        "central_control_boost": 0.15,  # Small boost for placing in central areas (increased)
        "edge_affinity_boost": 0.05,  # Small boost for placing near edges (if strategy calls for it)
    }

    # Max possible feature score for normalization (heuristic) [cite: 182]
    max_heuristic_feature_score = (4 * (FEATURE_WEIGHTS["identical_3"] + FEATURE_WEIGHTS["arithmetic_3"])) + \
                                  ...
                                  FEATURE_WEIGHTS["edge_affinity_boost"] [cite: 182]
    if max_heuristic_feature_score == 0: max_heuristic_feature_score = 1.0

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1:  # Only score empty cells [cite: 183]
                continue
            ...
            for p_val in potential_numbers_to_place:
                ...
                # Feature 1 & 2: Lines of 3 (identical or arithmetic) involving p_val [cite: 183]
                for dr_line, dc_line in [(0, 1), (1, 0), (1, 1), (1, -1)]:  # H, V, D1, D2 [cite: 183]
                    for offset in range(-2, 1):  # Window start relative to (r_idx,c_idx) [cite: 183]
                        line_values = []
                        is_valid_line = True
                        involved_pval = False
                        ...
                        for i in range(3):
                            ...
                            if 0 <= check_r < rows and 0 <= check_c < cols: [cite: 184]
                                line_values.append(temp_grid[check_r, check_c])
                            else:
                                is_valid_line = False
                                break

                        if is_valid_line and involved_pval and len(line_values) == 3 and all(v != -1 for v in line_values): [cite: 185]
                            s = line_values
                ...
                if rows > 1 and cols > 1:  # Only for grids larger than 1x1 [cite: 186]
                    center_r = (rows - 1) / 2.0
                    center_c = (cols - 1) / 2.0
                    dist_to_center = MathUtils().euclidean_distance((float(r_idx), float(c_idx)), (center_r, center_c)) [cite: 186]
                    # Simplified: closer to center gives more boost
                    current_features_score += FEATURE_WEIGHTS["central_control_boost"] * (1 - MathUtils().normalize_value(dist_to_center, 0, MathUtils().euclidean_distance((0, 0), (center_r, center_c)), clamp=True)) [cite: 186]

                    dist_to_edge = min(r_idx, rows - 1 - r_idx, c_idx, cols - 1 - c_idx) [cite: 186]
                    ...
                    if max_min_dist_to_edge > 0:
                        current_features_score += FEATURE_WEIGHTS["edge_affinity_boost"] * (1 - MathUtils().normalize_value(dist_to_edge, 0, max_min_dist_to_edge, clamp=True)) [cite: 186]
...
def EXT_GM19_Masked_Number_Skip_Pattern_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    輸出詮釋: 分數越高表示該空格若填入特定數字,能與盤面上觀察到的主要「跳格」規律性最
    為吻合。
    強化: 精確計算跳格模式並評估其強度，確保模式識別的準確性。
    """
    effective_request_id = request_id or "N/A_brain_GM19"
    logger.debug("Executing EXT_GM19_Masked_Number_Skip_Pattern_Vec",
                 extra={'request_id': effective_request_id})
    ...
    revealed_numbers_info = [  # List of {'value': val, 'r': r, 'c': c}
        ...
        for r in range(rows)
        for c in range(cols)
        if grid[r, c] != -1 and grid[r, c] > 0  # Assuming positive numbers are actual game numbers [cite: 187]
    ]

    if not revealed_numbers_info: return scores  # No numbers to analyze pattern from [cite: 187]

    expected_max_number_on_card = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 187]
    base_positions: Dict[int, Tuple[int, int]] = {}  # value -> (expected_r, expected_c) [cite: 188]
    for k_val in range(1, expected_max_number_on_card + 1):
        base_r = (k_val - 1) // cols
        base_c = (k_val - 1) % cols
        if base_r < rows:  # Ensure base position is within grid dimensions [cite: 188]
            base_positions[k_val] = (base_r, base_c)

    skip_vectors: Dict[int, Tuple[int, int]] = {}  # value -> (delta_r, delta_col) [cite: 188]
    for rn_info in revealed_numbers_info:
        val = rn_info['value']
        if val in base_positions: [cite: 188]
            expected_r, expected_c = base_positions[val] [cite: 189]
            skip_vectors[val] = (rn_info['r'] - expected_r, rn_info['c'] - expected_c)

    if not skip_vectors: return scores

    dominant_skip_patterns_strength: Dict[Tuple[int, int], float] = {}  # (dr,dc) -> strength [cite: 189]
    if skip_vectors:
        ...
        # Pattern needs some support [cite: 189]
        min_occurrences_for_pattern = max(1, int(len(skip_vector_tuples_list) * 0.05))
        # Adjusted from 2, to allow even single unique skips if they are the only ones. [cite: 190]

        for skip_vec_tuple, count_val in counts.most_common():
            if count_val >= min_occurrences_for_pattern:
                # Strength could simply be normalized count [cite: 190]
    ...
    potential_numbers_to_place_set = BoardAnalyzerUtils().get_legal_values_for_placement(grid) [cite: 190]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1: continue [cite: 190]
            ...
            for p_val_test in potential_numbers_to_place_set:
                ...
                for current_skip_pattern, pattern_str in dominant_skip_patterns_strength.items():
                    ...
                    if predicted_r == r_idx and predicted_c == c_idx: [cite: 190]
                        # This empty cell (r_idx, c_idx) is where p_val_test would land if it followed this skip pattern [cite: 191]
                        current_score_fit = pattern_str  # Score is strength of the pattern it fits [cite: 191]
                        if current_score_fit > cell_max_pattern_score:
                            cell_max_pattern_score = current_score_fit

            scores[r_idx, c_idx] = cell_max_pattern_score  # Max score if multiple patterns/values fit this cell [cite: 191]
    return scores
...
def EXT_GM20_Skip_Pattern_Confidence_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    revealed_numbers_info_gm20 = [] [cite: 192]
    for r in range(rows):
        for c in range(cols):
            if grid[r, c] != -1 and grid[r, c] > 0: [cite: 192]
                revealed_numbers_info_gm20.append({'value': int(grid[r, c]), 'r': r, 'c': c})

    if not revealed_numbers_info_gm20: return scores

    expected_max_num_gm20 = BoardAnalyzerUtils().get_card_max_value_from_grid_dimensions((rows, cols)) [cite: 192]
    base_pos_gm20: Dict[int, Tuple[int, int]] = {
        k: ((k - 1) // cols, (k - 1) % cols) for k in range(1, expected_max_num_gm20 + 1) if ((k - 1) // cols) < rows
    } [cite: 192]

    skip_vecs_initial_gm20: Dict[int, Tuple[int, int]] = {} [cite: 193]
    for rn in revealed_numbers_info_gm20:
        val = rn['value']
        if val in base_pos_gm20:
            skip_vecs_initial_gm20[val] = (rn['r'] - base_pos_gm20[val][0], rn['c'] - base_pos_gm20[val][1]) [cite: 193]

    dominant_patterns_details_gm20: List[Dict[str, Any]] = []  # List of {'skip': (dr,dc), 'values': [sorted_values_in_pattern], 'strength': float} [cite: 193]
    ...
    potential_nums_to_place_gm20 = BoardAnalyzerUtils().get_legal_values_for_placement(grid) [cite: 193]

    for r_idx in range(rows):
        for c_idx in range(cols):
            if grid[r_idx, c_idx] != -1: continue [cite: 193]
            ...
            for p_val_test in potential_nums_to_place_gm20:
                ...
                base_r_t, base_c_t = base_pos_gm20[p_val_test]  # [cite: 194]
                ...
                for pattern_detail in dominant_patterns_details_gm20:
                    ...
                    predicted_r_for_pval = base_r_t + pat_skip_dr [cite: 195]
                    predicted_c_for_pval = base_c_t + pat_skip_dc

                    if predicted_r_for_pval == r_idx and predicted_c_for_pval == c_idx:  # Geometrically fits [cite: 195]
                        enhancement_factor = 0.5  # Base for geometric fit related to pattern strength

                        # Check for arithmetic sequence enhancement [cite: 195]
                        if len(pat_existing_vals) >= 1:  # Need at least one existing number to extend/mend [cite: 195]
                            ...
                            if len(temp_sequence_with_pval) >= 2:  # Need at least 2 for a diff
                                ...
                                if len(diffs_in_temp_seq) > 0:
                                    # Check if it becomes more consistently arithmetic [cite: 195]
                                    is_arithmetic_now = len(set(diffs_in_temp_seq)) == 1  # All diffs are the same [cite: 195]
                                    first_diff = diffs_in_temp_seq[0]

                                    if is_arithmetic_now and first_diff != 0:  # It forms a new, consistent arithmetic sequence [cite: 196]
                                        enhancement_factor += 0.4  # Strong enhancement

                                    elif len(temp_sequence_with_pval) >= 3:  # Check if it mends a specific gap [cite: 196]
                                        # E.g. pat_existing_vals = [2,6], p_val_test = 4 => temp = [2,4,6] [cite: 197]
                                        # This is covered if the whole sequence is_arithmetic_now. [cite: 198]
                                        # Could add bonus if p_val_test is between min/max of pat_existing_vals [cite: 198]
            ...
            scores[r_idx, c_idx] = MathUtils().normalize_value(max_confidence_score_for_cell_gm20, 0, 1.0, clamp=True) [cite: 199]
    return scores
...
if __name__ == '__main__':
    ...
    try:
        scores = get_module_score(module_to_test, dummy_grid) [cite: 200]
    ...
    try:
        scores_gm1 = get_module_score("EXT_GM1_Row_Control_Vec", grid_gm1_test) [cite: 200]
        print(f"Scores for EXT_GM1_Row_Control_Vec:\n{scores_gm1}")
    except ValueError as e:
        print(f"Error: {e}")

    # Test EXT_F10_Discontinuity_Vec for sequence completion
    print("\nTesting EXT_F10_Discontinuity_Vec for sequence completion...") [cite: 200]
    ...
    try:
        scores_f10 = get_module_score("EXT_F10_Discontinuity_Vec", grid_f10_test) [cite: 200]
    ...
    print(f"\nTesting get_module_score with non-existent module '{non_existent_module}'...") [cite: 201]
    try:
        scores = get_module_score(non_existent_module, dummy_grid) [cite: 201]
    ...
    for i, name in enumerate(REGISTERED_MODULES_BRAIN.keys()): [cite: 202]
        print(f"{i+1}. {name}") [cite: 202]

    print(f"\nTotal modules registered: {len(REGISTERED_MODULES_BRAIN)}") [cite: 202]
    print("\nbrain.py verification complete.") [cite: 203]