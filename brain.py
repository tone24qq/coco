...
class MathUtils:
    ...
    def normalize_value(self, value: float, min_val: float, max_val: float, clamp: bool = True) -> float:
        ...
        normalized = (value - min_val) / (max_val - min_val + 1e-10)  # Added epsilon to avoid division by zero
                ...
                if line[j] == -1:
                        ...
                        if line[k] == -1:
                            ...
                            if temp_gap_count <= allow_gaps:
                                ...
                                for l in range(k + 1, n):
                                    ...
                                    if math.isclose(line[l], expected_next, rel_tol=1e-9):
                    ...
                    for k in range(j + 1, n):
                        if line[k] == -1:
                            ...
                            continue
                        expected_next = current_seq_values[-1] + diff
                        if math.isclose(line[k], expected_next, rel_tol=1e-9):
...
def EXT_GM20_Skip_Pattern_Confidence_Vec(grid: np.ndarray, request_id: Optional[str] = "N/A") -> np.ndarray:
    ...
    for skip_v, count_v in counts.most_common():
        if count_v >= min_occ:
            ...
            dominant_patterns_details.append({"skip": skip_v, "values": pattern_vals, "strength": p_strength * 1.1})  # Boost pattern strength
        else:
            break
                        ...
                        if len(pat_existing_vals) >= 1:
                            ...
                            if len(temp_sequence) >= 2:
                                ...
                                if len(set(diffs)) == 1 and diffs[0] != 0:
                                    enhancement_factor += 0.5  # Increased enhancement for arithmetic sequences
                                elif len(temp_sequence) >= 3 and min(pat_existing_vals) < p_val_test < max(pat_existing_vals):
                                    enhancement_factor += 0.15  # Adjusted enhancement for in-sequence values
