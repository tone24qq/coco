...
為確保主流程穩定性與可擴充性，請遵守以下原則：
	•	禁止擅自刪除或更改現有功能、模組、註冊函式，除非新設計能完整涵蓋舊邏輯並提升效能、準確性或延展性。
	•	若為重構或優化，請保持原邏輯功能不變，並經過單元測試驗證。
	•	新增模組或公式應以 不干擾主流程 為原則，並透過註冊機制（如 @register_strategy, @register_formula）掛載。
	•	除非重疊，否則原模組不可刪除，舊版應保留作為備援或實驗比對。

違反上述規定的提交會被退回，或需經專案管理者（如橘子）審核。